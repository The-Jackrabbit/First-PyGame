...
def tick(keys):
    ...
    state_x = character.x
    state_y = character.y
    camera.draw(gamebox.from_text(character.x + 20, 15, "X: {}".format(str(int(state_x))), "Arial", 20, "Black", bold=True))
    camera.draw(gamebox.from_text(character.x + 20, 40, "Y: {}".format(str(int(state_y))), "Arial", 20, "Black", bold=True))
    '''
    if health == 0:
        x_coord = character.x
